	...
	public:
		...
		void setIdColor(){
			...
			if ((parity % 2) == 0){					//偶数のパリティビットのときのみ値を読み込む
				this->id = (this->buf >> 3);		//上位2bitをidとする
...
int main(int argc, char *argv[]){
	...
	while (1){
		...
		for (int PointNum = 0; PointNum < LightMax-1; PointNum++){		//display bin to 
			...
			putText(disp2, "id=" + to_string(point[PointNum].getId()) + ":color=" + to_string(point[PointNum].getColor()) + ":debug=" + point[PointNum].debug(), Point(DifDisplayX*point[PointNum].getX(), DifDisplayY*point[PointNum].getY()), FONT_HERSHEY_COMPLEX, 0.5, Scalar(200, 200, 200));
