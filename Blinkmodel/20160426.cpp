...
double  DifDisplayX = DispFrameWidth / FrameWidth;		//xy座標にこれをかけるとdispでの座標になる
	...
	while (1){
		...
		case 1:				//PONGモード
			...
			for (int i = 0; i < ballMax - 1; i++){
				ballDat[i][0] += ballDat[i][2];			//加速度を加算
				ballDat[i][1] += ballDat[i][3];

				if (enableEdgeBounce){					//壁反射
					int tempx = ballDat[i][0] + ballDat[i][2];
					int tempy = ballDat[i][1] + ballDat[i][3];
					if ((tempx < ballRadious) || (tempx>FrameWidth - ballRadious)) ballDat[i][2] = -1 * ballDat[i][2];		//ボールが次フレームに画面外なら加速度を反転
